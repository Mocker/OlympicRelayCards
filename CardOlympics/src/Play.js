 ...
 * --------------------------------------------------
 * Choose racers with different stats
 * As they run, play their cards at the right time for the win
 * 
 * made for Summer Slow Jams 2026 - Luck game jam
 * --------------------------------------------------
 */
export class Play extends Phaser.Scene {
    // All cards names
    cardNames = CARD_DATABASE.map(card => card.id);
    // Cards Game Objects
    cards = [];
    ...
    createGridCards() {
        ...
        return gridCardNames.map((name, index) => {
            // Find the matching card config in the database
            const cardData = CARD_DATABASE.find(c => c.id === name);

            const newCard = new Card({
                scene: this,
                x: this.gridConfiguration.x + (98 + this.gridConfiguration.paddingX) * (index % 4),
                y: -1000,
                frontTexture: cardData.texture,
                cardName: cardData.id,
                stats: cardData.stats // Attach custom stats automatically!
            });
